...
const Sidebar = ( {isMenuOpen, setIsMenuOpen} ) => {
    ...
    const { chats , setSelectedChat, theme ,setTheme , user , navigate , createNewChat,
       axios , setChats, fetchUsersChat , setToken , token }  = useAppContext()
...
{/* chat button */}
         <button onClick={createNewChat} className = 'flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81f7] text-sm rounded-md cursor-pointer'>
            ...
            <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
              ...
              chats.filter((chat)=> chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
                <div onClick={()=>{navigate('/'); setSelectedChat(chat); setIsMenuOpen(false)}}
                   key={chat.id} className='p-2 px-4 dark:bg-[#57317C]/25 border border-gray-300 dark:border-[#80609F]/20 rounded-md cursor-pointer flex justify-between group'>
                    <div>
