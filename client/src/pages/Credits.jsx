...
const purchasePlan = async (planId) => {
  try {
    const {data} = await axios.post('/api/credit/purches' , {planId} , 
      {headers: {Authorization: token}})
    if(data.success){
    ...
    sm:px-6 lg:px-8 py-12'>
      ...
      text-gray-800 dark:text-white'>Credit Plans</h2>
       <div className='flex flex-wrap justify-center gap-8'>
        {plans.map((plan)=>(
          <div key={plan._id} className={`border border-gray-300 
          dark:border-purple-700 rounded-lg shadow hover:shadow-lg 
          transition-shadow p-6 min-w-[300px] flex flex-col
           ${plan._id === "pro" ? "bg-purple-100 dark:bg-purple-900":"bg-white dark:bg-transparent"}`}>
            <div className='flex-1'>
              <h3 className='text-xl font-semibold text-gray-900 dark:text-white mb-2'>
                {plan.name}
                 ...
                 <ul className='list-dist list-inside text-sm text-gray-700 
            ...
            text-white font-medium py-2 rounded transition-color cursor-pointer'>Buy Now</button>
